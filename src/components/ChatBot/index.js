        ...
        <div className="fixed bottom-8 right-8 z-50 flex flex-col items-end sm:items-auto sm:justify-end">
            {showFloatingButton && !isOpen && (
                <div className="relative group">
                    <div className={`absolute right-14 bottom-1/2 translate-y-1/2 hidden group-hover:block text-xs px-3 py-2 rounded shadow-md w-48 
            ...
            <AnimatePresence onExitComplete={() => setShowFloatingButton(true)}>
                {isOpen && (
                    <motion.div
                        ...
                        className={`
                            fixed inset-0 sm:static sm:w-[420px] h-full sm:h-[85vh]
                            border sm:rounded-2xl shadow-lg flex flex-col overflow-hidden z-50
                            ${isDark ? "bg-gray-900 text-white border-gray-600" : "bg-white text-gray-800 border-gray-300"}
                        `}
                    >
                        {/* Header */}
                        <div className={`px-4 py-3 flex justify-between items-center text-sm font-semibold shadow-sm relative
                        ...
                        <div className={`flex-1 overflow-y-auto px-4 py-3 space-y-3 text-sm 
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {quickReplies.map((q, i) => (
                                            <button
                                                ...
                                                className={`text-left px-4 py-2 rounded-full text-sm transition
                                                    ${isDark
                            ...
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                                    <div className={`px-4 py-3 rounded-2xl max-w-[80%] text-sm leading-relaxed whitespace-pre-line shadow-sm
                                        ${msg.role === "user"
                            ...
                            {loading && (
                                <div className="flex items-end space-x-2">
                                    ...
                                    <div className={`flex gap-1 items-center px-4 py-2 rounded-2xl shadow-sm
                        ...
                        <div className={`flex items-center gap-2 px-3 py-2 border-t 
                            ...
                            <input
                                ...
                                disabled={loading}
                                className={`flex-1 text-sm px-3 py-2 rounded-full outline-none disabled:opacity-50
